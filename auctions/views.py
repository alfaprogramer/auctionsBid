...
def endAuction(request, id):
    listingData = Listing.objects.get(pk=id)
    listingData.activeStatus = False
    listingData.save()
    is_owner = request.user.username == listingData.owner.username
    isListingInWatchlist = request.user in listingData.watchlist.all()
    allComments = Comment.objects.filter(listing=listingData)
...
def index(request):
    activeListinings = Listing.objects.filter(activeStatus=True )
    allCategories = Category.objects.all()
    return render(request, "auctions/index.html", {
        "listings" : activeListinings,
...
def showCategory(request):
    if request.method == "POST":
        ...
        activeListinings = Listing.objects.filter(activeStatus=True, category=category )
        allCategories = Category.objects.all()
        return render(request, "auctions/index.html", {
        "listings" : activeListinings,
        "categories" : allCategories
